...
# --- POMDP (calibration meta-control) ---
from calibration_pomdp import (
    build_calibration_agent,
    discretize_calib_obs,
    CalibAction,
    CalibObs,
    CalibState
)
...
def main():
    ...
    with SpheroEduAPI(toy) as droid:
        ...
        # --- Calibration POMDP setup ---
        use_calib_pomdp = True
        ...
        try:
            while True:
                ...
                # --- POMDP logging defaults (will be overwritten if POMDP is used) ---
                calib_belief_ok = float("nan")
                calib_belief_drift = float("nan")
                ...
                # --- Calibration POMDP (meta-control) ---
                if use_calib_pomdp:
                    pe = prediction_error
                    if isinstance(pe, float) and math.isnan(pe):
                        pe = 20.0  # treat as medium error when NaN

                    calib_obs_idx = discretize_calib_obs(
                        prediction_error_px=pe,
                        stuck_frames=stuck_frames,
                        safety_recent=prev_safety_active,
                    )

                    # Inference
                    qs = calib_agent.infer_states([calib_obs_idx])
                    calib_agent.infer_policies()
                    calib_action_idx = calib_agent.sample_action()[0]

                    # Extract belief over states for logging
                    qs_state = qs[0]  # 1-factor model -> single array
                    calib_belief_ok = float(qs_state[CalibState.OK])
                    calib_belief_drift = float(qs_state[CalibState.DRIFT])
                    calib_belief_bad = float(qs_state[CalibState.BAD])

                    # Map action to effective_max_speed
                    effective_max_speed = MAX_SPEED
                    if calib_action_idx == CalibAction.CAUTION:
                        effective_max_speed = int(MAX_SPEED * 0.7)

                    # Optional: console logging every N frames
                    if loop_idx % 50 == 0:
                        # Human-readable obs and action labels
                        try:
                            obs_name = CalibObs(calib_obs_idx).name
                        except Exception:
                            obs_name = f"{calib_obs_idx}"
                        try:
                            action_name = CalibAction(calib_action_idx).name
                        except Exception:
                            action_name = f"{calib_action_idx}"

                        print(
                            f"[POMDP] loop={loop_idx} "
                            f"obs={obs_name} action={action_name} "
                            f"belief=[OK:{calib_belief_ok:.2f}, "
                            f"DRIFT:{calib_belief_drift:.2f}, "
                            f"BAD:{calib_belief_bad:.2f}] "
                            f"eff_max_speed={effective_max_speed}"
                        )
                else:
                    effective_max_speed = MAX_SPEED
                ...
                if will_exit or is_near_boundary(est_x, est_y, w, h):
                    ...
                    # respect POMDP speed cap as well
                    speed_cmd = min(speed_cmd, effective_max_speed)
                ...
                last_loop_time = now
                prev_safety_active = safety_active  # for POMDP
