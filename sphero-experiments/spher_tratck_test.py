...
VISION_LATENCY_SEC = 0.18    # Approx camera+processing latency (tune!)
...
def main():
    ...
    with SpheroEduAPI(toy) as droid:
        ...
        print("--- VISION-BASED SERVO WITH ROBUST BALL DETECTION, LATENCY, SAFETY & LOGGING ---")
        ...
        try:
            while True:
                ...
                pred_x_for_log = float("nan")
                pred_y_for_log = float("nan")
                ...
                if ball_found:
                    ...
                    # prediction: measurement + measured velocity * latency
                    pred_x = bx + meas_vx * VISION_LATENCY_SEC
                    pred_y = by + meas_vy * VISION_LATENCY_SEC
                    pred_x_for_log = pred_x
                    pred_y_for_log = pred_y

                    # low-gain blend with previous estimate
                    ALPHA = 0.3
                    est_x = (1-ALPHA) * est_x + ALPHA * pred_x
                    est_y = (1-ALPHA) * est_y + ALPHA * pred_y
                    est_vx, est_vy = meas_vx, meas_vy

                    # log prediction error (vs raw measurement)
                ...
                if will_exit or is_near_boundary(est_x, est_y, w, h):
                    ...
                    T_BRAKE = 0.3  # seconds; tune if needed
                ...
                log_writer.writerow([
                    ...
                    int(safety_active),
                    stuck_frames,
                    # NEW latency-related columns
                    pred_x_for_log, pred_y_for_log,
...
# # --- LATENCY ESTIMATION ---
# LATENCY_INIT = 0.3          # Initial guess (seconds)
...
# # --- NAVIGATION & SAFETY ---
# MAX_SPEED = 30               # You said you slowed it down
# MIN_SPEED = 5
# TARGET_REACHED_PIX = 35
# BORDER_MARGIN = 100          # Bigger safe margin in small field
# PREDICT_HORIZON_SEC = 0.4    # For bounds check (can be larger than prediction horizon)
# CENTER_RETURN_SPEED = 25
...
#         print("--- VISION-BASED SERVO WITH ROBUST DETECTION, ONLINE LATENCY, SAFETY & LOGGING ---")
...
#                     # log prediction error (vs raw ball position)
#                     err = math.sqrt((est_x - bx)**2 + (est_y - by)**2)
#                     pred_errors.append(err)
#                     if len(pred_errors) > 200:
#                         pred_errors.pop(0)
...
#                 # --- Apply command & update latency-events & stuck ---
...
#                 cv2.putText(vis, f"Est: ({int(est_x)}, {int(est_y)})  Goal: {goal_label}",
#                             (20, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,0), 2)
#                 cv2.putText(vis, f"Dist goal: {int(dist_to_goal)} px  Latency est: {int(latency_est*1000)} ms",
#                             (20, 135), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200,200,200), 1)
#                 cv2.putText(vis, f"Stuck frames: {stuck_frames}",
#                             (20, 155), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,165,255), 1)
